...
- 레이어도 이에 맞춰서 7\*7은 conv2D 2개, 9\*9는 conv2D 3개로 수정.
...
- 아이템 3개까지 훈련시키고, 저장되는 gif 파일을 확인해 봤을 때, 위에서 확인했던 에이전트 시야의 문제점 때문에 우리가 원하는 최단거리에 대한 결과는 얻기 어렵다고 판단.
- 하지만 아이템 개수를 늘려가는 방법이 효과가 있다는 것은 확인!
## 6.2 일 기준 PATCH ✒️

- 아이템 개수를 늘려가는 방법을, 아이템 시야를 잡기전
