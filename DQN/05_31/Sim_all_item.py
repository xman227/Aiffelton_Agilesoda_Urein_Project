...
goal_reward = 1
finish_reward = 10
print('reward:' , move_reward, obs_reward, goal_reward, finish_reward)
...
class Simulator:
    ...
    def set_box(self):
        ...
        #self.grid[self.local_target[0][0]][self.local_target[0][1]] = 3
    ...
    def reset(self, epi):
        ...
        self.grid = np.ones((self.height, self.width), dtype="int8")
        ...
        self.curloc = [9, 4]
        self.grid[int(self.curloc[0])][int(self.curloc[1])] = 2
        
        self.done = False
       
        return np.eye(4)[self.grid]
            ...
            if [new_x, new_y] == [9, 4]:
                if self.terminal_location == [9, 4]:
                    ...
                    if len(self.actions) < 200:
                        print(f'200번 안에 들어왔다!!!!!! : {len(self.actions)}')
                        ...
                        print(f'{len(self.actions)}번 시도 끝에 성공,,, 고생했다..')
                        
        # 원핫 인코딩       
        
        return self.grid, np.eye(4)[self.grid], reward, self.cumulative_reward, self.done, goal_ob_reward
