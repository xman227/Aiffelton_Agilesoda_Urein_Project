...
class Simulator:
    ...
    def set_box(self):
        ...
        for order_box in order_csv.itertuples(index = True, name ='Pandas'):
            #self.grid[getattr(order_box, "row")][getattr(order_box, "col")] = 0.6
        ...
        self.grid[self.local_target[0][0]][self.local_target[0][1]] = 0.6 #첫번째 타겟만
    ...
    def reset(self, epi):
        ...
        self.curloc = [9, 4]
        self.grid[int(self.curloc[0])][int(self.curloc[1])] = 0.3
        ...
        if any(out_of_boundary):
            ...
            # 현재 목표에 도달한 경우
                ...
                # item 일때
                    ...
                    self.local_target.remove(self.local_target[0])
                    self.grid[self.local_target[0][0]][self.local_target[0][1]] = 0.6
                
                if self.item_loc: #저번에가 item 이었던 자리었으면
                    self.grid[cur_x][cur_y] = 0
                    self.grid[new_x][new_y] = 0.3
                else:
