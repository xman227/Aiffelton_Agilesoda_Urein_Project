...
item_rgb = (255/255, 255/255, 000/255)

search_rgb = (102/255, 102/255, 255/255)
...
class Render():
    def __init__(self, screen_width, screen_height, unit, start_point, data_path, log_path, local_target):
        ...
        self.draw_initial(data_path, type_="items")
        self.update_movement(start_point, 0, 'action')
    ...
    def draw_initial(self, data_path, type_):
        if type_ == "box":
            ...
            position_data = self.local_target
            position_data = pd.DataFrame(position_data, columns=['row', 'col'])
            color = item_rgb
        else:
            # TODO error
            pass
        ...
        for line in position_data.itertuples(index = True, name ='Pandas'):
            if type_ == "box":
                ...
                self.items.append((getattr(line, "row"), getattr(line, "col")))
                pass
    ...
    def update_movement(self, new_pos, idx, _type):
